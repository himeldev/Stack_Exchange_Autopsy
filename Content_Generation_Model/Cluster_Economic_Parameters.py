...
for row in csv_data:
    site.append(row[0])
    site_economic_parameters.append([float(row[1]), float(row[2]), float(row[3]), float(row[4]), float(row[5])])
...
for n_neighbors in range(2, 21):
    ...
    for n_components in range(1, 6):
        error = sklearn.manifold.Isomap(n_neighbors, n_components).fit(site_economic_parameters).reconstruction_error()
    ...
    fig.savefig('C:/Users/Himel/Documents/GitHub/Stack_Exchange_Autopsy/Figures/Cobb-Douglas/Isomap-Cluster/Isomap/Number_of_Neighbors_'+str(n_neighbors))
    plt.close(fig)
    figure_count += 1
...
reduced_data = manifold.Isomap(optimal_n_neighbors, optimal_n_components).fit_transform(site_economic_parameters)
print reduced_data
...
if optimal_n_components == 2:
    ...
    fig.savefig('C:/Users/Himel/Documents/GitHub/Stack_Exchange_Autopsy/Figures/Cobb-Douglas/Isomap-Cluster/Cluster/Cluster_Visualization')
    plt.close(fig)
...
for i in range(optimal_n_clusters):
    ...
    fig.tight_layout()
    fig.savefig('C:/Users/Himel/Documents/GitHub/Stack_Exchange_Autopsy/Figures/Cobb-Douglas/Isomap-Cluster/Health/Cluster_Health_C'+str(i+1))
    plt.close(fig)
    stat1[:] = []
    stat2[:] = []
    stat3[:] = []
